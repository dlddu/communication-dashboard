...
final class DatabaseSchemaTests: XCTestCase {
    ...
    // MARK: - widget_layout CHECK 제약 조건 테스트

    func test_widgetLayout_rejectsNegativePositionX() throws {
        // Arrange
        let dbQueue = try makeMigratedDatabase()

        // Act & Assert - position_x < 0은 CHECK 제약 조건 위반
        XCTAssertThrowsError(try dbQueue.write { db in
            try db.execute(
                sql: """
                INSERT INTO widget_layout (id, plugin_id, position_x, position_y, size, display_order)
                VALUES (?, ?, ?, ?, ?, ?)
                """,
                arguments: ["w-neg-x", "slack", -1, 0, "medium", 0]
            )
        })
    }

    func test_widgetLayout_rejectsNegativePositionY() throws {
        // Arrange
        let dbQueue = try makeMigratedDatabase()

        // Act & Assert - position_y < 0은 CHECK 제약 조건 위반
        XCTAssertThrowsError(try dbQueue.write { db in
            try db.execute(
                sql: """
                INSERT INTO widget_layout (id, plugin_id, position_x, position_y, size, display_order)
                VALUES (?, ?, ?, ?, ?, ?)
                """,
                arguments: ["w-neg-y", "slack", 0, -1, "medium", 0]
            )
        })
    }

    func test_widgetLayout_rejectsInvalidSize() throws {
        // Arrange
        let dbQueue = try makeMigratedDatabase()

        // Act & Assert - 유효하지 않은 size 값은 CHECK 제약 조건 위반
        XCTAssertThrowsError(try dbQueue.write { db in
            try db.execute(
                sql: """
                INSERT INTO widget_layout (id, plugin_id, position_x, position_y, size, display_order)
                VALUES (?, ?, ?, ?, ?, ?)
                """,
                arguments: ["w-bad-size", "slack", 0, 0, "extra-large", 0]
            )
        })
    }

    func test_widgetLayout_acceptsValidSizes() throws {
        // Arrange
        let dbQueue = try makeMigratedDatabase()

        // Act & Assert - small, medium, large 모두 허용
        XCTAssertNoThrow(try dbQueue.write { db in
            for (i, size) in ["small", "medium", "large"].enumerated() {
                try db.execute(
                    sql: """
                    INSERT INTO widget_layout (id, plugin_id, position_x, position_y, size, display_order)
                    VALUES (?, ?, ?, ?, ?, ?)
                    """,
                    arguments: ["w-size-\(i)", "slack", 0, 0, size, i]
                )
            }
        })
    }
    ...
    // MARK: - widget_layout CHECK 제약 조건 추가 테스트

    func test_widgetLayout_rejectsNegativeDisplayOrder() throws {
        // Arrange
        let dbQueue = try makeMigratedDatabase()

        // Act & Assert - display_order < 0은 CHECK 제약 조건 위반
        XCTAssertThrowsError(try dbQueue.write { db in
            try db.execute(
                sql: """
                INSERT INTO widget_layout (id, plugin_id, position_x, position_y, size, display_order)
                VALUES (?, ?, ?, ?, ?, ?)
                """,
                arguments: ["w-neg-order", "slack", 0, 0, "medium", -1]
            )
        })
    }

    // MARK: - notifications PRIMARY KEY 제약 조건 테스트

    func test_notifications_rejectsDuplicateId() throws {
        // Arrange
        let dbQueue = try makeMigratedDatabase()
        let record = NotificationRecord(
            id: "dup-001",
        ...
        // Assert - 두 번째 삽입은 PRIMARY KEY 위반으로 실패해야 함
        XCTAssertThrowsError(try dbQueue.write { db in
            var duplicate = record
            duplicate.title = "중복 알림"
            try duplicate.insert(db)
        })
    }

    // MARK: - widget_layout PRIMARY KEY 제약 조건 테스트

    func test_widgetLayout_rejectsDuplicateId() throws {
        // Arrange
        let dbQueue = try makeMigratedDatabase()
        let record = WidgetLayout(id: "dup-widget-001", pluginId: "slack")
