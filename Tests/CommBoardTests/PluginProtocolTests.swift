...
final class PluginProtocolTests: XCTestCase {
    ...
    func test_mockPlugin_isEnabled_defaultsToTrue() {
        // Arrange & Act
        let plugin = MockPlugin()

        // Assert
        XCTAssertTrue(plugin.isEnabled, "isEnabled 기본값은 true여야 합니다")
    }

    func test_mockPlugin_config_defaultsToEmptyDict() {
        // Arrange & Act
        let plugin = MockPlugin()

        // Assert
        XCTAssertTrue(plugin.config.isEmpty, "config 기본값은 빈 딕셔너리여야 합니다")
    }

    // MARK: - fetch callCount 초기 상태 테스트

    func test_mockPlugin_fetchCallCount_initiallyZero() {
        // Arrange & Act
        let plugin = MockPlugin()

        // Assert
        XCTAssertEqual(plugin.fetchCallCount, 0)
    }

    func test_mockPlugin_testConnectionCallCount_initiallyZero() {
        // Arrange & Act
        let plugin = MockPlugin()

        // Assert
        XCTAssertEqual(plugin.testConnectionCallCount, 0)
    }
}
